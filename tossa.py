...
class TopicSentimentAnalysis:
    ...
    def preprocess(self, version: str = None):
        ...
        self.preprocessed_reviews = [
            preprocessor.process(review)
            for review in map(lambda x: x.body,
                              filter(lambda x, version=version: version is None or x.version == version, self.reviews))
        ]
    ...
    def build_w2v_model(self, save_w2v=False):
    ...
    def calc_senti_words(self, pwords: List[str] = None, nwords: List[str] = None):
    ...
    def topic_modeling(self, n_topics: int, gram_mode: str):
        ...
        _, self.coherence_value, self.doc_topics, self.topic_words = coherence_lda(
            ...
            gram_mode=gram_mode)
        if self.save_intermediate:
            self._dump_intermediate("coherence_value.pkl", self.coherence_value)
    ...
    def prepare_summary(self, n_keywords: int = 4):
        ...
        for topic in topic_summary:
            ...
            # topic_keywords
            topic[2] = [kw[0] for kw in sorted(topic[3], key=lambda x: x[1], reverse=True)[:n_keywords]]
