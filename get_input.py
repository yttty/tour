...
def get_num_label(fp_label, fp_num, fp_emerging, fp_label_sen, fp_emer_sen):
    ...
    for idx, raw in enumerate(topic_label):
        ...
        clean_labels = "; ".join(
            [" ".join(l.split("_")) for l in labels if "_" in l])
        ...
        if emergs != ["None"]:
            clean_emers = "; ".join(
                [" ".join(l.split("_")) for l in emergs if "_" in l])
        else:
            clean_emers = "0"
        ...
        sent_labels = "".join([
            str(int(idx / 2) + 1) + ": " + sent + ";<br>"
            for idx, sent in enumerate(sents.split("\t")) if (idx % 2 == 0)
        ][:-1])

        # process emerging sentences
        try:
            emergs = "".join(emer_sen[idx -
                                      emeg_num].split(":")[1:]).strip(" \n")
            if emergs != "None":
                sent_labels = "".join([
                    str(int(idx / 2) + 1) + ": " + sent + ";<br>"
                    for idx, sent in enumerate(emergs.split("\t"))
                    if (idx % 2 == 0)
                ][:-1])
        except IndexError:
            pass

        input_dict[version].append(
            ("topic" + number, nums[int(number)], clean_labels, clean_emers,
             version, sent_labels, str(len(versions))))

        if number not in topic_dict:
            topic_dict[number] = []
        topic_dict[number].append(
            ("topic" + number, nums[int(number)], clean_labels, clean_emers,
             version, sent_labels, str(len(versions))))
